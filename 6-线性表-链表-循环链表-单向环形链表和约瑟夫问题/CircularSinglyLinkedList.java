...
public class CircularSinglyLinkedList {
    ...
    public void countBoy(int startNo, int countNum, int nums) {
        ...
        // 创建辅助指针，帮助小孩出圈
        Boy helper = first;
        // 需要创建一个辅助指针（变量）helper，事先应该指向环形链表的最后的结点
        while (true) {
            if (helper.getNext() == first) // 说明 helper 指向最后小孩结点
                break;
            helper = helper.getNext();
        }

        // 小孩报数前，先让 first 和 helper 移动 k – 1 次，让 first 指向 startNo 位置
...
class Boy {
    private int no; // 编号
    private Boy next; // 指向下一个结点，默认 null
